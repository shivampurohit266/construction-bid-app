...
class Invoice extends Component {
  ...
  agreementList = async () => {
    ...
    fetch(
      `${url}/api/agreement/by_id/${this.props.match.params.id}`,
      requestOptions
    )
  ...
  invoiceData = async () => {
    ...
    if (acc.length && acc.length > 0) {
      const { invoice_account_number } = acc[0];
      this.setState({
        acc_no: invoice_account_number,
      });
    }
  };
  ...
  getdraft = async () => {
    const token = localStorage.getItem("token");
    await getData(
      `${url}/api/invoice/getby/${this.props.match.params.tender}`,
      token
    )
      .then((result) => {
  ...
  handleSubmit = async (event) => {
    ...
    data.set("agree_id", this.state.agree_id);
    data.set("agreement", this.state.agreement);
    data.set("agreement_id", this.state.agreement_id);
    data.set("date", this.state.date);
    data.set("due_date", this.state.due_date);
    data.set("invoice_number", this.state.inv_no);
    data.set("reference", this.state.refer);
    ...
    if (
      ...
      // if (this.state.client_type === "Client") {
      //   data.set("type", "user");
      // } else {
      //   data.set("type", "resource");
      // }
    ...
    // if (this.props.match.url == `/invoice/${this.props.match.params?.tender}/draft`) {
    //   if (this.state.client_type === "user") {
    //     data.set("type", "user");
    //   } else {
    //     data.set("type", "resource");
                    ...
                    <div style={{ maxWidth: "610px" }}>
                      ...
                      <div className="form-group">
                        <div className="row align-items-center">
                          ...
                          <div className="col-md-6">
                            {this.state.emails && this.state.emails.length && typeof this.state.emails === 'Array'
                                  ...
                                  <tr className="text-right">
                                    ...
                                    <td id="5result">
                                      {this.state.row_phase.length <= 0
                                        ...
                                        : this.state.sub_total}
                                      {this.state.row_phase.length <= 0 &&
                                      this.state.sub_total === undefined
                                        ? 0
                                        : this.state.sub_total}
                                    </td>
                                  </tr>
  ...
  return (
    <tr className="text-right i-val customerIDCell">
      ...
      <td
        ...
        contentEditable="true"
      >
        {props.val?.items} {props.val?.val} {props.val?.des}
      </td>
      ...
      <td
        ...
        contentEditable="true"
      >
        {props.val.price ? props.val.price : 1}
      </td>
      ...
      <td
        ...
        contentEditable="true"
      >
        {props.val.unit ? props.val.unit : props.val.cost}
      </td>
      <td className="mat_cost1" colSpan="2">
        {props.val.amount ? props.val.amount : props.val.cost}
      </td>
    </tr>
  );
};

...
const SummaryInvoices = ({ t, projectPlanning }) => {
    ...
    <tr key={i} style={{ background: i % 2 === 0 ? '#f3f3f3' : 'white' }}>
      ...
      <td style={{ display: windowSize <= 479 ? 'none' : '' }}>
        <div className='table-cell'>
          ...
          <p className='table-cell-value'>
            {' '}
            {x.sent === 1 ? 'Sent' : x.sent === 0 ? 'Draft' : null}
          </p>
        </div>
      </td>
      <td>
        <div className='table-cell'>
          ...
          <p className='table-cell-value'>
            <button
              onClick={(e) =>
                window.open(
                  `${projectPlanning.invoice_pdf_url}${x.invoice_pdf}`,
                  '_blank'
                )
              }
              type='button'
  ...
  return (
    <div className='card-body'>
      {projectPlanning === undefined || !projectPlanning ? (
        <div>No data available</div>
      ) : (
        <>
          <h3>{t('projectManagment.Manage.invoices_summary')}</h3>
...
export default withTranslation()(SummaryInvoices);

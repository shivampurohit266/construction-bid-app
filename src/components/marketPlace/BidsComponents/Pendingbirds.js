...
class Pendingbirds extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  render() {
    ...
    const { t, i18n } = this.props;
    const pending = birds?.filter(item => item.tb_status == 2)
    const indexOfLastPost = this.state.currentPage * this.state.postsPerPage;
    const indexOfFirstPost = indexOfLastPost - this.state.postsPerPage;
    const currentPosts = pending?.slice(indexOfFirstPost, indexOfLastPost);
    const length = pending ? pending.length : "";
    return (
      <div className="card">
                ...
                <div className="row">
                  ...
                  <div className="col-sm-4" >
                    {/* {
                                            ...
                                            } className="btn btn-primary button_bid">  {t("mybid.Submit_Proposal")}  </button> : 
                                            <button type="button" className="btn btn-secondary button_bid">Withdraw</button>
                                        } */}
                    ...
                    >
                      {t("my_contracts.submit_proposal")}
                    </Link>) : data.tb_status !== 2 && data.tb_status !== 4 ? <>
                    ...
                    {/* {
                                            data.tb_status == 7 ? <button type="button" className="btn btn-secondary button_bid">{t("mybid.Withdraw")} </button> : ''
                                        } */}
...
export default withTranslation()(Pendingbirds);
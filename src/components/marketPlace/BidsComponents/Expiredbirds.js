...
class Expiredbirds extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  render() {
    ...
    const { t, i18n } = this.props;
    const Expired = birds?.filter(item => item.tb_status == 4)
    const indexOfLastPost = this.state.currentPage * this.state.postsPerPage;
    const indexOfFirstPost = indexOfLastPost - this.state.postsPerPage;
    const currentPosts = Expired?.slice(indexOfFirstPost, indexOfLastPost);
    const length = Expired ? Expired.length : "";
    ...
    return (
      <div className="card">
        {currentPosts?.length > 0 ? (
              ...
              <div className="card-body" key={index} style={{ borderBottom: "1px solid lightgray" }}>
                <div className="row">
                  ...
                  <div className="col-sm-4" >
                    ...
                    {/* {
                      data.tb_status == 7 ? <button type="button" className="btn btn-secondary button_bid">Withdraw</button> : ''
                    } */}
          ...
          //  <div style={{ textAlign: "center", height: "50px", marginTop: "15px" }}> <Spinner animation="border" role="status">
...
export default withTranslation()(Expiredbirds);
...
class Hiredbirds extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  render() {
    ...
    const { t, i18n } = this.props;
    const Hired = birds?.filter(item => item.tb_status == 3)
    const indexOfLastPost = this.state.currentPage * this.state.postsPerPage;
    const indexOfFirstPost = indexOfLastPost - this.state.postsPerPage;
    const currentPosts = Hired?.slice(indexOfFirstPost, indexOfLastPost);
    const length = Hired ? Hired.length : "";
...
export default withTranslation()(Hiredbirds);